...
export function ReimbursementView({
  ...
  return (
    <div className="space-y-4">
                ...
                >
                  ...
                  <div className="flex items-center justify-between sm:justify-end space-x-3">
                    ...
                    {exp.reimbursementStatus === 'PENDING' ? (
                      ...
                      <button
                        onClick={() => setRevertTargetExpense(exp)}
      ...
      {/* Confirmation Modal for Reverting Reimbursement */}
      {revertTargetExpense && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-900">¿Revertir comprobante a Pendiente?</h4>
                <p className="text-xs text-slate-500 mt-1">
                  Se reabrirá el reintegro de <strong>{revertTargetExpense.vendor}</strong> por <strong>{formatCurrency(revertTargetExpense.amount, revertTargetExpense.currency)}</strong>.
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-2 pt-2">
              ...
              <button
                type="button"
                onClick={() => {
                  const id = revertTargetExpense.id;
                  setRevertTargetExpense(null);
                  onToggleReimbursementStatus(id);
                }}
